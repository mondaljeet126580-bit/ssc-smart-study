(() => {
  ...
  function connect() {
    ...
    socket.onopen = () => {
      window.postMessage({ source: 'JEET_DELTA_BRIDGE_PAGE', type: 'status', connected: true }, '*');
    };
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.data.source !== 'JEET_DELTA_BRIDGE_EXTENSION' || event.data.type !== 'config') return;
