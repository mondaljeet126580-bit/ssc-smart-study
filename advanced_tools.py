...
# Persistent only for the lifetime of this Render process. Delta Exchange does not
# expose a REST endpoint for TradingView-style chart drawings, so these levels are
# server-side annotations, not native Delta account drawings.
...
def register_advanced_tools(mcp, client) -> None:
    ...
    def _render(clean, symbol, resolution, requested, show_trendline, show_sr, label="Horizontal level"):
        ...
        levels = _levels(symbol, resolution, requested)
        rmin, rmax = min(lows), max(highs)
        ...
        for frac in range(6):
            y = int(top + frac/5*plot_h); p = pmax - frac/5*(pmax-pmin)
            draw.line((left,y,width-right,y), fill="#e5e7eb", width=1); draw.text((width-right+10,y-9),f"{p:.6g}",fill="#374151",font=axis_font)
        ...
        for i,row in enumerate(clean):
            x=x_at(i); yo,yc,yh,yl=y_at(row["open"]),y_at(row["close"]),y_at(row["high"]),y_at(row["low"]); fill="#16a34a" if row["close"]>=row["open"] else "#dc2626"
            draw.line((x,yh,x,yl),fill=fill,width=2); tb,bb=min(yo,yc),max(yo,yc); bb=max(bb,tb+2); draw.rectangle((x-body_half,tb,x+body_half,bb),fill=fill,outline=fill)
        ...
        line_y=None
        for lv in levels:
            yy=max(top,min(height-bottom,y_at(lv))); is_current=requested is not None and math.isclose(lv,requested,rel_tol=0.0,abs_tol=1e-12)
            draw.line((left,yy,width-right,yy),fill="#7c3aed",width=4 if is_current else 3)
            txt = label if is_current else "Stored level"
            draw.text((left+8,max(top+4,min(height-bottom-22,yy+6))),f"{txt}: {lv:.15g}",fill="#7c3aed",font=small_font)
            if is_current: line_y=yy
    ...
    def _json_chart(image_bytes, summary):
        result = dict(summary); result["image_base64"] = base64.b64encode(image_bytes).decode("ascii"); result["image_format"]="png"; return result
    ...
    async def plot_market_chart(symbol: str, resolution: str = "15m", candles: int = 120, horizontal_price: float | None = None, show_trendline: bool = True, show_support_resistance: bool = True) -> dict[str, Any]:
        """Render a chart image with server-side stored annotations. Does not modify the native Delta UI."""
    ...
    async def plot_horizontal_price_line(symbol: str, price: float, resolution: str = "15m", candles: int = 120, show_trendline: bool = True, show_support_resistance: bool = True, label: str = "Horizontal level") -> dict[str, Any]:
        """Create and persist a server-side horizontal chart annotation at exactly the supplied price.

        Delta Exchange's REST/WebSocket API does not expose a native chart-drawing endpoint;
        this tool therefore cannot place an object into the native Delta/TradingView chart UI.
        It persists the requested level in this Render process and re-renders it on subsequent chart calls.
        """
        ...
        return {**_json_chart(image_bytes,summary),"tool":"plot_horizontal_price_line","line_price_exact":format(exact_price,'.15g'),"line_action":"server_side_chart_annotation","native_delta_chart_modified":False,"message":f"Stored and rendered horizontal level at exactly {format(exact_price,'.15g')} on {symbol}.","note":"Native Delta/TradingView chart drawing requires a client-side/browser bridge because Delta API has no chart-drawing endpoint."}

    @mcp.tool()
    async def get_chart_drawings(symbol: str, resolution: str = "15m") -> dict[str, Any]:
        """Return server-side stored horizontal annotations for a symbol/timeframe."""
        symbol=symbol.strip().upper(); resolution=resolution.strip()
        if resolution not in _RESOLUTION_SECONDS: raise ValueError(f"Unsupported resolution: {resolution}")
        levels=_levels(symbol,resolution,None)
        return {"symbol":symbol,"resolution":resolution,"levels":levels,"native_delta_chart_modified":False,"source":"Render MCP server state"}
